...
fn main() {
    ...
    for i in xs {
        handles.push(thread::spawn(move || map_data(&i)));
	}
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    let temp_partition_size = v.len() / num_partitions;
    let mut remainder = v.len() % num_partitions;
    let mut xs: Vec<Vec<usize>> = Vec::new();
    let mut counter = 0;
    for _partition in 0..num_partitions {
        let mut partition_size = temp_partition_size;
        if remainder > 0 {
            remainder -= 1;
            partition_size += 1;
		}
        let mut x : Vec<usize> = Vec::new();
        for i in counter..(counter + partition_size) {
            x.push(v[i]);
		}
        xs.push(x);
